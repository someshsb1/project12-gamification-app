...
if [[ "$1" == "setup" ]] 
then
    sudo docker run -e "ACCEPT_EULA=Y" -e "MSSQL_SA_PASSWORD=Tu1ippetal!" -p 1433:1433 --name tulip_db --hostname tulip_db --platform linux/amd64 -d mcr.microsoft.com/mssql/server:2022-latest
elif [[ "$1" == "start" ]] 
then
    sudo docker start tulip_db
elif [[ "$1" == "stop" ]] 
then
    sudo docker stop tulip_db
elif [[ "$1" == "delete" ]] 
then
    sudo docker stop tulip_db && docker rm tulip_db
else
